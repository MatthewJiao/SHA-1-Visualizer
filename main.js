...
function sha() {
  ...
  function sha1(text) {
    ...
    const asciiStuff = text.split('').map((letter) => letter.charCodeAt(0)); 
  
    let binaryStuff = asciiStuff.map((num) => num.toString(2));
  
    for(var i = 0; i < binaryStuff.length; i++){
      let temp = binaryStuff[i].toString().split('');
       
      while (temp.length < 8) {
        temp.unshift('0');
      }
      binaryStuff[i] =  temp.join('');
    }
  
    var x = binaryStuff.join('');
    let numString = x + '1';
    ...
    var moreZerosRabbit = binaryLength.toString();
  
    while(moreZerosRabbit.length<64){
      moreZerosRabbit = '0' + moreZerosRabbit;
    }
  
    numString += moreZerosRabbit; 
